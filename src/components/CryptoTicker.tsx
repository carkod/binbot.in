    ...
    <div className="fixed top-20 left-0 right-0 z-40 h-9 bg-[#252422] border-b border-white/10 overflow-hidden flex items-center">
      ...
      <div className="ticker-track flex items-center gap-0 whitespace-nowrap">
        {items.map((coin, i) => {
          ...
          const up = change >= 0;
          return (
            <span key={`${coin.id}-${i}`} className="flex items-center gap-1.5 px-5 text-[11px] font-medium border-r border-white/10 last:border-r-0">
              <span className="text-white/50 uppercase tracking-widest">{coin.symbol}</span>
              <span className="text-white/90">${formatPrice(coin.current_price)}</span>
              <span className={`flex items-center gap-0.5 ${up ? "text-[#6bd098]" : "text-red-400"}`}>
